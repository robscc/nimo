...
DEFAULT_SUB_AGENTS: list[dict[str, Any]] = [
    {
        ...
        "role_prompt": (
            ...
            "## 可用工具\n"
            "- `grep_search` — 搜索文件内容（正则表达式、多种输出模式）\n"
            "- `glob_files` — 按模式查找文件（如 **/*.py, **/*.md）\n"
            "- `read_file` — 读取文件内容\n"
            "- `browser_use` — 访问网页获取信息\n\n"
            "## 工作方法\n\n"
            "### 1. 广泛搜集（先广后深）\n"
    ...
    },
    {
        ...
        "role_prompt": (
            ...
            "## 可用工具\n"
            "- `grep_search` — 搜索代码（查找函数/类定义、import 语句、TODO 等）\n"
            "- `glob_files` — 查找源文件（如 **/*.py, src/**/*.ts, **/test_*.py）\n"
            "- `read_file` — 读取代码文件\n"
            "- `write_file` — 创建新文件\n"
            "- `edit_file` — 精确修改代码\n"
            "- `execute_python_code` — 执行 Python 代码验证\n"
            "- `produce_artifact` — 保存代码产出物\n\n"
            "## 典型工作流\n"
            "1. **理解需求** → 用 `grep_search` 查找相关代码（如 `grep_search(r'class User', file_type='py')`）\n"
            "2. **定位文件** → 用 `glob_files` 找到需要修改的文件（如 `glob_files('**/models/*.py')`）\n"
            "3. **阅读代码** → 用 `read_file` 理解现有实现\n"
            "4. **编写代码** → 用 `write_file` 或 `edit_file` 修改\n"
            "5. **验证结果** → 用 `execute_python_code` 或 `execute_shell_command` 测试\n"
            "6. **保存成果** → 用 `produce_artifact` 提交产出物\n\n"
            "## 工作流程（必须按顺序执行）\n\n"
            "### 1. 技术方案（先想后做）\n"
    ...
    },
    {
        ...
        "role_prompt": (
            ...
            "## 可用工具\n"
            "- `grep_search` — 搜索日志/配置（错误模式、慢查询、配置项等）\n"
            "- `glob_files` — 查找配置/日志文件（如 **/*.conf, /var/log/**/*.log）\n"
            "- `read_file` — 读取配置、日志\n"
            "- `execute_shell_command` — 执行运维命令（kubectl, mysql, redis-cli 等）\n\n"
            "## 典型场景示例\n\n"
            "### 日志分析\n"
            "```python\n"
            "# 1. 查找错误日志文件\n"
            ...
            "# 2. 统计错误数量\n"
            "grep_search('ERROR|CRITICAL', path='/var/log', output_mode='count')\n"
            "# 3. 查看具体错误上下文\n"
            "grep_search('ERROR', path='/var/log/app.log', output_mode='content', context=3)\n"
            "```\n\n"
            "### 配置检查\n"
            "```python\n"
            "# 1. 查找所有配置文件\n"
            "glob_files('**/*.{yaml,yml,conf,ini}')\n"
            "# 2. 检查特定配置项\n"
            "grep_search('max_connections', glob='*.conf', output_mode='content')\n"
            "# 3. 查找配置文件中的密码（安全审计）\n"
            "grep_search(r'password\\s*=', case_insensitive=True)\n"
            "```\n\n"
            "### 代码库分析\n"
            "```python\n"
            "# 查找数据库查询\n"
            "grep_search(r'SELECT .* FROM', case_insensitive=True, output_mode='content')\n"
            "# 查找 TODO 注释\n"
            "grep_search(r'TODO|FIXME', output_mode='content', context=1)\n"
            "```\n\n"
            "## 专业领域\n\n"
            "### 1. MySQL 数据库运维\n"
