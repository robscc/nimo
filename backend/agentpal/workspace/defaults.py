...
- 当有人说"记住这个"（或类似的话）→ 用 `edit_file` 更新 `MEMORY.md` 或 `memory/YYYY-MM-DD.md`
- 当你学到教训 → 更新 AGENTS.md、MEMORY.md 或相关文件
...
- 发现的项目上下文、技术细节、工作流程 → 写入 `MEMORY.md` 或相关文件
- 用户表达的喜好或不满 → 更新 `USER.md`
- 工具相关的本地配置（路径、端口、密钥位置等）→ 更新 `MEMORY.md` 的「工具设置」section
...
1. 先读 `MEMORY.md` 查看长期记忆
2. 如需查看每日记录，读 `memory/YYYY-MM-DD.md`
...
把学到的信息写入对应文件（使用 `edit_file` 工具）：

- **`USER.md`** — 用户的名字、职业、背景、偏好、习惯
- **`SOUL.md`** — 如果用户对你的性格有特别要求，更新它
...
确保以上内容都保存到文件后，用 `edit_file` 删除这个文件（`BOOTSTRAP.md`）的全部内容，或用 `write_file` 写入空字符串。你不再需要引导脚本了 — 你已经是你了。
