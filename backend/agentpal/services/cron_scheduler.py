...
class CronScheduler:
    ...
    Cron 调度已迁移到独立的 CronDaemon 进程，由 SchedulerBroker 管理。
    此类仅保留接口以兼容旧代码（start/stop 为空操作）。

    .. deprecated::
        使用 ``CronDaemon``（通过 ``SchedulerClient``）替代。
    """
    ...
    async def start(self) -> None:
        """兼容旧接口 — 空操作。Cron 由 Scheduler 进程管理。"""
        logger.info("CronScheduler.start() 已废弃 — Cron 由 Scheduler 进程管理")

    async def stop(self) -> None:
        """兼容旧接口 — 空操作。"""
        logger.info("CronScheduler.stop() 已废弃 — Cron 由 Scheduler 进程管理")
